...
//Java：找出两数组的不同
class T2215_FindTheDifferenceOfTwoArrays{
    public static void main(String[] args) {
        Solution solution = new T2215_FindTheDifferenceOfTwoArrays().new Solution();
...
class Solution {
    public List<List<Integer>> findDifference(int[] nums1, int[] nums2) {
        ...
        list.add(set.stream().collect(Collectors.toList()));
        for (int j = 0 ; j < nums2.length ; j++) {
            if (!num1List.contains(nums2[j])) {
                set1.add(nums2[j]);
            }
        }
        list.add(set1.stream().collect(Collectors.toList()));
        return list;
    }
}
